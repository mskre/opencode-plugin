...
**Default model:** `openai/gpt-5.6-luna` with the `xhigh` variant for the built-in `build` agent. The built-in `plan` agent uses `openai/gpt-5.6-sol` with the `high` variant.
