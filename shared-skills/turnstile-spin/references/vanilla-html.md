...
For static sites or any project without a JS framework. The widget renders client-side; the form submits to its existing backend. Apply the input validation, timeout, trusted-IP, and fail-closed checks in [integration](integration.md#backend-gate) to the backend examples below.
...
If there is no existing server-side handler or form-host verification hook, report that boundary. Adding a Worker, Pages Function, or form host is a separate task, not part of this integration.

## Substitutions

| Placeholder         | Replace with                                                         |
| ------------------- | -------------------------------------------------------------------- |
| `YOUR_SITEKEY`      | The confirmed existing or newly provisioned sitekey |
| `/api/subscribe`    | The path to your existing form-handling endpoint                     |
| `TURNSTILE_SECRET`  | Secret stored via [provisioning](provisioning.md) or [guarded recovery](credential-recovery.md); never inline it |
