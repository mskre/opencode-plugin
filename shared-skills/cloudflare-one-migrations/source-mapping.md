...
- Confirm the account's current Access destination limit before creating large app segments. Request a supported increase or split apps with identical policies; bundled historical limits (5 by default, up to 50 by request) are not proof of current account entitlement.
