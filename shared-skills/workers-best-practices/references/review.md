...
Use project-generated types and the installed package for the deployed compatibility target. When investigating an upgrade or a newly documented API, compare published types without replacing the project's version:
...
Runtime examples are in [rules](rules.md). Type-specific issues:
...
`npx tsc --noEmit` checks the project's types; its configured `no-floating-promises` lint rule checks promise handling. Examples advertised as executable need imports and configuration, not only plausible API names.
