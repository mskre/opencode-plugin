# Wrangler Command Catalogue

Read the relevant section, not the whole catalogue. Examples target Wrangler v4; verify flags with installed `wrangler <command> --help` and [current docs](https://developers.cloudflare.com/workers/wrangler/).

## Contents

- [New Worker](#quick-start-new-worker), [core commands](#quick-reference-core-commands), [configuration](#configuration-wranglerjsonc)
- [Local development](#local-development), [deployment and secrets](#deployment)
- [KV](#kv-key-value-store), [R2](#r2-object-storage), [D1](#d1-sql-database), [Vectorize](#vectorize-vector-database), [Hyperdrive](#hyperdrive-database-accelerator)
- [Workers AI](#workers-ai), [Queues](#queues), [Containers](#containers), [Workflows](#workflows), [Pipelines](#pipelines), [Secrets Store](#secrets-store)
- [Pages](#pages-frontend-deployment), [observability](#observability), [testing](#testing), [troubleshooting](#troubleshooting)

Resolve account, environment, config, and local/remote target before mutations. Destructive commands need approval. Local dev can reach real resources through remote bindings; Workers AI incurs charges.
Secret values must not appear in arguments, logs, source, or shell history. Use interactive prompts or stdin/file-based secret input; do not commit secret files.
...
```bash
# Discover supported credential input for this version before create/update.
wrangler hyperdrive create --help
wrangler hyperdrive update --help
...
`create` supports origin host/port/database/user or a connection string; `update` can rotate credentials. Shell expansion of `$DB_PASSWORD` or `$HYPERDRIVE_CONNECTION_STRING` still exposes secrets in process arguments. If the installed CLI has no secure input mechanism, use the approved dashboard or secret-management path rather than those flags.
