...
description: Use only to generate website design-reference images when the current harness exposes a real image-generation tool. Produces images, not frontend code. Prefer one readable horizontal image per requested section and a consistent visual system across the set.
compatibility: Requires an available image-generation tool; this skill does not create images through text alone.
---

# CAPABILITY GATE

Verify that a real image-generation tool is available before planning output. If it is unavailable, state that the task cannot run in the current harness. Do not simulate image calls, substitute stock images, or claim generated files exist.
